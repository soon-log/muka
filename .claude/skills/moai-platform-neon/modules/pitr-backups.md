...
async function restoreToPoint(timestamp: Date, branchName?: string) {
  const name =
    branchName || `restore-${timestamp.toISOString().replace(/[:.]/g, '-')}`;
...
async function performRestore(
  ...
  if (validateFirst) {
    ...
    if (targetTimestamp < oldestAllowed) {
      throw new Error(
        `Timestamp ${targetTimestamp.toISOString()} is outside ${retentionDays}-day retention period`
      );
    }
...
async function validateRestore(
  ...
  // Compare row counts
  const [originalCount] =
    await originalDb`SELECT COUNT(*) as count FROM ${tableName}`;
  const [restoredCount] =
    await restoredDb`SELECT COUNT(*) as count FROM ${tableName}`;

  console.log(
    `Original count: ${originalCount.count}, Restored count: ${restoredCount.count}`
  );
...
async function verifyRestore(
  ...
  return {
    ...
    overallStatus:
      failedChecks === 0
        ? 'success'
        : failedChecks < criticalTables.length / 2
          ? 'warning'
          : 'failure',
    issues,
  };
...
const restored = await performRestore(safeTime, {
  branchName: 'emergency-restore',
});
```
